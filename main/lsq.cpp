...
int main() {
    ...
    ofstream outfile("spp_results.txt");
    if (!outfile.is_open()) {
        cout << "Failed to create output file." << endl;
        return -1;
    }

    outfile << "Epoch, Time, Latitude(deg), Longitude(deg), Height(m), Sats" << endl;
    ...
    while (oem7.getNextEpoch(obs)) {
        ...
        for (int iter = 0; iter < 10; ++iter) {
            ...
            for (auto& it : obs.satTypeValueData) {
                ...
                if (sat.system == "G" && oem7.latestGps.count(sat.id)) eph = &oem7.latestGps.at(sat.id);
                else if (sat.system == "C" && oem7.latestBds.count(sat.id)) eph = &oem7.latestBds.at(sat.id);
                
                if (!eph) continue;

                double tau = pr / C_MPS;
                CommonTime t_emit = obs.epoch;
                t_emit.m_sod -= (tau + x_state[3] / C_MPS);
                ...
                Eigen::Vector3d satPos = pvt.p;
                Eigen::Vector3d correctedSatPos;
                ...
                double rho = (correctedSatPos - x_state.head<3>()).norm();
                if (rho < 1.0) rho = 20000000.0;

                double elev = 90.0;
                if (x_state.head<3>().norm() > 1000.0) {
                    try { elev = elevation(XYZ(x_state.head<3>()), XYZ(correctedSatPos)); } catch (...) { elev = 0.0; }
                }
                
                if (elev < 10.0 && x_state.head<3>().norm() > 1000.0) continue;

                double tropo = (x_state.head<3>().norm() > 1000.0) ? getTropDelay(elev) : 0.0;

                EquID eid(sat, "Obs");
                EquData ed;
                ed.prefit = pr - (rho + x_state[3] - C_MPS * dts - C_MPS * rel + tropo);
                ed.varCoeffData[varX] = -(correctedSatPos.x() - x_state.x()) / rho;
                ed.varCoeffData[varY] = -(correctedSatPos.y() - x_state.y()) / rho;
                ed.varCoeffData[varZ] = -(correctedSatPos.z() - x_state.z()) / rho;
            ...
            try {
                ...
                Eigen::Vector3d dx = solverLSQ.getdxyz();
